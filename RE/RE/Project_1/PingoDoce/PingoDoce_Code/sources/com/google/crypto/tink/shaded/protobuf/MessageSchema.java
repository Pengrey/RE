...
public final class MessageSchema<T> implements Schema<T> {
    ...
    /* loaded from: classes.dex */
    public static /* synthetic */ class C41141 {
    ...
    private int decodeMapEntryValue(byte[] bArr, int i, int i2, WireFormat.FieldType fieldType, Class<?> cls, ArrayDecoders.Registers registers) throws IOException {
        switch (C41141.$SwitchMap$com$google$protobuf$WireFormat$FieldType[fieldType.ordinal()]) {
            case 1:
    ...
    private int getSerializedSizeProto2(T t) {
        ...
        while (i4 < this.buffer.length) {
            ...
            if (type <= 17) {
                ...
                i = (!this.useCachedSizeField || type < FieldType.DOUBLE_LIST_PACKED.m28489id() || type > FieldType.SINT64_LIST_PACKED.m28489id()) ? 0 : this.buffer[i4 + 2] & OFFSET_MASK;
                i2 = 0;
            }
    ...
    private int getSerializedSizeProto3(T t) {
        ...
        for (int i2 = 0; i2 < this.buffer.length; i2 += 3) {
            ...
            long offset = offset(typeAndOffsetAt);
            int i3 = (type < FieldType.DOUBLE_LIST_PACKED.m28489id() || type > FieldType.SINT64_LIST_PACKED.m28489id()) ? 0 : this.buffer[i2 + 2] & OFFSET_MASK;
            switch (type) {
                case 0:
    ...
    static <T> MessageSchema<T> newSchemaForMessageInfo(StructuralMessageInfo structuralMessageInfo, NewInstanceSchema newInstanceSchema, ListFieldSchema listFieldSchema, UnknownFieldSchema<?, ?> unknownFieldSchema, ExtensionSchema<?> extensionSchema, MapFieldSchema mapFieldSchema) {
        ...
        for (FieldInfo fieldInfo : fields) {
            if (fieldInfo.getType() == FieldType.MAP) {
                i2++;
            } else if (fieldInfo.getType().m28489id() >= 18 && fieldInfo.getType().m28489id() <= 49) {
                i3++;
            }
        }
        ...
        while (i4 < fields.length) {
            ...
            if (fieldInfo2.getType() == FieldType.MAP) {
                iArr2[i7] = i5;
                i7++;
            } else if (fieldInfo2.getType().m28489id() >= 18 && fieldInfo2.getType().m28489id() <= 49) {
                i = i5;
        ...
        switch (i7) {
            ...
            case 59:
                if (i5 == 2) {
                    ...
                    if (i13 == 0) {
                        unsafe.putObject(t, j, BuildConfig.VERSION_NAME);
